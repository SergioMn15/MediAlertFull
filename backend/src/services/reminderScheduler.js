...
async function processDatabaseReminders(app) {
  const result = await query(
    `SELECT p.id AS patient_id, p.name, p.email, p.phone, p.reminder_channel, p.reminder_opt_in,
            pr.id AS prescription_id, pr.issued_at,
  ...
  for (const row of result.rows) {
    ...
    const alreadyLogged = await hasLoggedScheduledReminderDb(item.id, scheduledAt);
    if (alreadyLogged) continue;

    const outcome = await sendReminderNotification({
      id: row.patient_id,
      name: row.name,
      email: row.email,
      phone: row.phone,
    ...
    if (outcome.status !== 'skipped') {
      console.log(`[Reminder] ${row.name} - ${item.name} -> ${outcome.status} (${outcome.provider})`);
    }
...
async function processDemoReminders(app) {
  ...
  for (const patient of patients) {
    ...
    if (!demo.notificationLogs[patient.id]) {
      demo.notificationLogs[patient.id] = [];
    }
    ...
    for (const item of activePrescription.items || []) {
      ...
      if (hasLoggedScheduledReminderDemo(demo.notificationLogs[patient.id], item.id, scheduledAt.toISOString())) {
        continue;
      }
...
function startReminderScheduler(app) {
  if (app.get('reminderSchedulerStarted')) {
    return;
  }

  const isEnabled = process.env.REMINDER_SCHEDULER_ENABLED === 'true';
  if (!isEnabled) {
    console.log('Scheduler de recordatorios desactivado. Usa REMINDER_SCHEDULER_ENABLED=true para activarlo.');
    app.set('reminderSchedulerStarted', true);
    return;
  }

  const intervalMs = Number(process.env.REMINDER_INTERVAL_MS || 15000);
  let isRunning = false;
